...
def getrem(name):
    return name if '_rem.' in name else None
...
def setOutputName(name, outputformat):
    if not os.path.isfile(f'{name}_rem.{outputformat}'):
        return f'{name}_rem.{outputformat}'
    else:
        for i in range(30):
            if not os.path.isfile(f'{name}_{str(i)}_rem.{outputformat}'):
                return f'{name}_{str(i)}_rem.{outputformat}'
    ...
    try:
        ...
        for line in process.stdout:
            if "error" in line.lower():
                ...
                return f"{clearname}_rem.{_output}"
        ...
        if len(todel)>0 or len(failed)>0 or len(completed)>0:
            ...
            if _delorig == "True":
                ...
                if typeconv == 'folder':
                    ...
                    torename = [os.path.normpath(f'{os.path.splitext(_folder)[0]}_rem.{_output}')]
                for file in torename:
                    if f"_rem.{_output}" in file:
                        os.rename(file, file.replace("_rem.","."))
